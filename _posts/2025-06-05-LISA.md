...
> Example from the paper: when someone says "Change the TV channel," a robot doesn't understand. Instead, it needs a command like "go to the table, find the remote, press the channel button." LISA introduces reasoning to solve such issues.
...
```
\mathcal{L} = \lambda_{txt} \cdot \mathcal{L}_{txt} + \lambda_{mask} \cdot \mathcal{L}_{mask}\\
\mathcal{L}_{txt}:\text{ Text generation loss (Auto-regressive CE)}\\
\mathcal{L}_{mask}:\text{ Mask loss = BCE + DICE}\\
\lambda_{txt}, \lambda_{mask}: \text{hyperparameters}
```

#### 📉 1. Text Generation Loss `\mathcal{L}_{txt}`
...
```
\mathcal{L}_{txt} = CE(\hat{y}_{txt}, y_{txt})
```

#### 📉 2. Mask Loss `\mathcal{L}_{mask}`
...
```
\mathcal{L}_{mask} = \lambda_{bce} \cdot BCE(\hat{M}, M) + \lambda_{dice} \cdot DICE(\hat{M}, M)
```
