...
![Image](https://github.com/user-attachments/assets/dc9d6d1f-7c52-4728-a6c8-3e4c5c9b9a88)
...
- (segmentation) 이미지를 `CLIP`으로 인코딩 → `CLIP2SAM`으로 `SAM`에 얼라인 → SAM Decoder에서 segmentation 결과추출  - (Recognizing) 그 segmentation 결과 → `SAM2CLIP` → 객채 이름 추출  
...
\[
L_{distill} = \mathrm{MSE}(F_{sam}, A_{sam2clip}(\mathrm{Fusion}(E_I^i)))
\]
